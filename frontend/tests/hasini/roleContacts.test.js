...
describe('getRoleContacts', () => {
  ...
  test('a role nobody has filled in yet resolves to an empty block, not undefined', () => {
    ...
    expect(getRoleContacts('contractor')).toEqual({});
  ...
  // manager, admin and inspector. A role added here without a matching entry on
  // the route would render an empty page after a silent 403.
  test('only the roles the staff endpoint admits ask for staff rows', () => {
    ...
    expect(withStaff.sort()).toEqual(['admin', 'inspector', 'manager']);
  });
});
