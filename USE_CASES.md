...
~~Re-seed the checklist to the 25 paper items~~ (migration `026`); ~~add `servicing_date`~~ (`inspections.serviced_at`, migration `027`) ~~, `address`, `town_council`~~ (on `lifts`, migration `028`); ~~render by paper section~~; ~~capture the inspector signature at submit~~; ~~enforce G1–G5~~ — ~~implement the zero-defect auto-file branch~~ (G6) — all done. Remaining: Branch A's `Pending Assignment` status (records with defects are still created `Open`), the UC-014 email at submit, and the A1 warning when the spot-check is more than a day after the servicing date.
...
8. Admin drills into the job-level table — latest 10 or all, sortable on every column, with an outliers-only filter — then exports to CSV, prints a snapshot, or exports a server-rendered PowerPoint deck (`POST /api/export/admin-costs-pptx`, admin-only).
...
- **A3 — Partial current month excluded from the forecast and the top-mover callout.** Comparing a half-finished month against complete ones would report artificial movement. The mover also ignores any category that was under 5% of the prior month's spend — a percentage swing on a near-zero base is arithmetic, not a finding.
